...
public class TestTrigger : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            GameManager.Instance.EnterMiniGame("MiniGame_02"); // Lobby → MiniGame_02
        }

        if (Input.GetKeyDown(KeyCode.Alpha3))
        {
            GameManager.Instance.EnterMiniGame("MiniGame_03"); // Lobby → MiniGame_03
        }

        if (Input.GetKeyDown(KeyCode.LeftArrow))
        {
            GameManager.Instance.ReturnToLobby(); // 미니게임 → Lobby (1,2,3 공통)
        }
