...
public class HalfSuccessManager : MonoBehaviour
{
    ...
    [SerializeField] private AudioClip HalfSuccessBGM;
    ...
    IEnumerator EndingStart()
    {
        ...
        yield return Dialogue("조상님", "열심히 안 한 건 아닌데, 그렇다고 눈물겹게 열심히 한 것도 아니고...", ancestorGod);

        yield return Dialogue("조상님", "딱 주 5일 턱걸이로 출근 도장만 찍은 느낌이구나...", ancestorGod);

        yield return Dialogue("조상님", "그래도 이 팍팍한 세상에 평타라도 친 게 어디냐.", ancestorGod);

        yield return Dialogue("조상님", "네 성의를 봐서 대박 복권까지는 아니어도, 로또 3등 당첨권을 내려주마!", ancestorGod);

        yield return Dialogue("조상님", "감질나느냐? 억울하면 다음엔 눈 딱 감고 풀악셀로 덕 한번 쌓아보거라!", ancestorGod);
    ...
    private void PlayHalfSuccessBGM()
    {
        if (bgmAudioSource != null && HalfSuccessBGM != null)
        {
            bgmAudioSource.clip = HalfSuccessBGM;
            bgmAudioSource.Play();
        }
    }
